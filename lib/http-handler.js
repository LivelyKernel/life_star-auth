...
req.on("data", function(d) { console.log(String(d));})
  if (!data) {
...
HTTPHandler.prototype.handleRequest = function(req, res, next) {
  ...
  this.whenInitialized(function() {
    ...
    var isOK = req.session && req.session[config.cookieField] && req.session[config.cookieField].username === "robertkrahn";
    this.userDB.findUserByCookie(req.session[config.cookieField], function(err, user) {
      if (err) res.status(500).end(String(err));
