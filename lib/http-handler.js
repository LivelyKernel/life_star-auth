...
LoginPage.tryLogin = function(authHandler, req, res) {
  var data = req.body;
  if (!data) {
    res.redirect(authHandler.config.paths.login + '?note=Login failed!');
  } else {
    authHandler.userDB.checkPassword(data.username, data.password, function(err, user) {
      if (err) res.status(500).end(String(err))
      else if (!user) res.redirect(authHandler.config.paths.login + '?note=Login%20failed!');
      else {
...
LoginPage.logout = function(authHandler, req, res) {
  req.session && (delete req.session[authHandler.config.authCookieField]);
  res.end("Logged out");
}
...
HTTPHandler.prototype.handleRequest = function(req, res, next) {
  ...
  var isAuthRequest = _.chain(paths.paths).values().some(function(path) {
    return new RegExp("^" + path).test(req.path); }).value();

  // e.g. login, let the http auth handler deal with the request
  console.log(req.path + "..." + isAuthRequest);
  if (isAuthRequest) { next(); return; }

  var isOK = req.session && req.session[config.authCookieField] && req.session[config.authCookieField].username === "robertkrahn";

  this.userDB.findUserByCookie(req.session[config.authCookieField], function(err, user) {
    if (err) res.status(500).end(String(err));
...
HTTPHandler.prototype.rememberUser = function(user, req) {
  var cookie = req.session[this.config.authCookieField] || (req.session[this.config.authCookieField] = {});
