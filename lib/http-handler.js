...
LoginPage.tryRegister = function(authHandler, req, res) {
  var data = req.body;
  if (!data) {
    res.redirect(authHandler.config.paths.register + '?note=Registering failed, no user data received!');
  } else {
    authHandler.userDB.register(data, function(err, user) {
      if (err || !user) res.redirect(
          authHandler.config.paths.register + '?note=' + (String(err) || 'Registering failed!'));
      else {
        authHandler.rememberUser(user, req);
        res.redirect(data.redirect || "/welcome.html");
      }
...
LoginPage.renderCurrentUserInfo = function(authHandler, req, res) {
  var authCookie = req.session && req.session[authHandler.config.cookieField];
  if (!authCookie) return res.json({error: "not logged in"});

  authHandler.userDB.findUserByCookie(authCookie, function(err, user) {
    res.json(err || !user ?
...
LoginPage.modifyCurrentUserInfo = function(authHandler, req, res) {
  var authCookie = req.session && req.session[authHandler.config.cookieField];
  var userData = req.body;
  var db = authHandler.userDB;
  ...
  async.waterfall([
    ...
    db.findUserByName.bind(db, authCookie.username),
      ...
      if (err || !updatedUser) return res.status(400).json({error: err ? String(err) : "Could not update user " + authCookie.username});
        ...
        authHandler.rememberUser(updatedUser, req);
...
HTTPHandler.prototype.registerWith = function(app, server) {
  ...
  app.get(this.config.paths.currentUser, function(req, res, next) { LoginPage.renderCurrentUserInfo(self, req, res); });
  app.post(this.config.paths.currentUser, function(req, res, next) { LoginPage.modifyCurrentUserInfo(self, req, res); });
